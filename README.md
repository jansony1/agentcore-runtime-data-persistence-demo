...
| Reuse same VM vs new VM | reuse → `POST https://{same-endpoint}/` (same `microvmId`, same disk); new → `run_microvm(...)` again (fresh isolated VM). You hold the endpoint↔VM mapping | reuse → `invoke_agent_runtime(..., runtimeSessionId="sess-abc")` (same id, same session); new → call with a different `runtimeSessionId`. Routed automatically |

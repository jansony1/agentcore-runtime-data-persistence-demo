...
| Reuse same VM vs new VM | **you choose explicitly**: POST again to the **same endpoint** (same `microvmId` → same disk/state) to reuse; call **`run_microvm` again** for a brand-new isolated VM. The endpoint↔VM mapping is yours to hold | **by `runtimeSessionId`**: reuse the same id → same compute/session; use a new id → a new isolated session. Routing is automatic |
